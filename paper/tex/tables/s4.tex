...
\begin{tabular}{@{}lllrr@{}}
\toprule
\makecell[l]{Analysis} & \makecell[l]{Stratum} & \makecell[l]{Model} & \makecell[r]{Clinical need added\\(95\% interval)} & \makecell[r]{Area and marital added\\(95\% interval)} \\
\midrule
...
\end{tabular}
\par\smallskip
\begin{minipage}{\linewidth}\scriptsize Stage-free: summary stage removed from the clinical block, risk strata unchanged. Year as categories: logistic regression only, one indicator per year replacing linear year and the 2020 indicator. Both use per-step tuning and the first fold seed. Primary estimates are in Table 2 of the main text. Specified after an internal review (protocol amendment 1.9).\end{minipage}
\end{table}
