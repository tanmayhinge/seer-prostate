...
\end{tabular}
\par\smallskip
\begin{minipage}{\linewidth}\scriptsize Skill is the percentage reduction in out-of-fold log-loss against the step 0 model (year of diagnosis only); added values are in percentage points. Intervals are 95\% cluster bootstrap intervals over 79 rurality by county income cells (500 resamples of out-of-fold predictions, no refitting), so they exclude model-fitting variability. \textsuperscript{a}Range of the area and marital increment over 10 fold assignments, from the robustness analysis with per-step tuning (protocol amendment 1.9). AUC: 0.5 is chance.\end{minipage}
\end{table*}
